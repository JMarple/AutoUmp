        ...
        uint8_t* buffer)
{
	...
	printf("\n");
	uint16_t interUpper = buffer[0];
    uint16_t interLower = buffer[1];
...
int main(int argc, char** argv)
{
	...
	const int32_t sizeObjects = 250*9; // 250 objects * 12 bytes to represent the center
    ...
    while (1==1)
    {
		...
		// get intersection (if there is one)
		while (indexInter < 2)
		{
			int len = RS232_PollComport(COM_PORT, &(intersectionBuffer[indexInter]), 3 - indexInter);
			indexInter += len;
		}

		uint16_t intersection = unpackIntersection(intersectionBuffer);	
